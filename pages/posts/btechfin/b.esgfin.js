...
export default function Release({ BESGFIN }) {
  ...
  const currentPosts = BESGFIN.slice(indexOfFirstPost, indexOfLastPost); // 현재 페이지에 보여줄 게시글 목록

  // 페이지 번호 목록을 만드는 함수
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(BESGFIN.length / postsPerPage); i++) {
    pageNumbers.push(i);
  }
