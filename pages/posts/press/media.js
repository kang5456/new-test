...
export default function Release({ media }) {
  ...
  const currentPosts = media.slice(indexOfFirstPost, indexOfLastPost); // 현재 페이지에 보여줄 게시글 목록

  // 페이지 번호 목록을 만드는 함수
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(media.length / postsPerPage); i++) {
    pageNumbers.push(i);
  }
                        ...
                        <Grid item key={fields.title} xs={12}>
                          <Press
                            ...
                            coverImage={
                              ...
                              (fields.youtube &&
                                getYoutubeThumbnailUrl(fields.youtube)) ||
