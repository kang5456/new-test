...
export async function getStaticProps() {
  ...
  const report = await getMoreInsight(null, "report"); // title 필요하지 않으면 null로
  return { revalidate: 1, props: { report } };
}
...
export default function Report({ report }) {
  ...
  const noPosts = report.length === 0;

  // 현재 페이지에 보여줄 게시글의 시작/끝 index 계산
  const indexOfLastPost = currentPage * postsPerPage;
  const indexOfFirstPost = indexOfLastPost - postsPerPage;
  const currentPosts = report.slice(indexOfFirstPost, indexOfLastPost); // 현재 페이지에 보여줄 게시글 목록

  // 페이지 번호 목록을 만드는 함수
  const pageNumbers = [];
  for (let i = 1; i <= Math.ceil(report.length / postsPerPage); i++) {
    pageNumbers.push(i);
  }
